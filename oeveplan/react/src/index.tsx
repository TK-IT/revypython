...
interface Part {name: string, kind: string, actor: string, singer: boolean}
interface Act {name: string, parts: Part[], kind: ActKind}
...
class Choice extends React.Component<ChoiceProps, {open: boolean}> {
    ...
    render() {
        ...
        const options = this.props.choices.map(
            c => {
                const classes = classNames({
                    [styles.choice_option]: true,
                    [styles.conflicts]: c.conflicts
                });
                return <div key={c.key === null ? 'null' : c.key} className={styles.dropdown_item}>
                    <a onClick={e => optionClick(c.key, e)}
...
class MultiChoice extends React.Component<MultiChoiceProps, {open: boolean, selected: boolean[]}> {
    state = {
        open: false,
        selected: [] as boolean[],
    }
    ...
    render() {
        ...
        const options = this.props.choices.map(
            (c, i) => <div key={c.key} className={styles.dropdown_item}>
                    <label>
                        <input type="checkbox"
                               onChange={() => onChange(i)}
...
class ActorChoice extends React.Component<ActorChoiceProps, {}> {
    render() {
        const choices = this.props.actors.map(
            actor => ({key: actor, name: actor, conflicts: false})
        );
        if (this.props.blank) {
            choices.unshift({key: '', name: '---', conflicts: false});
        }
        return <Choice choices={choices} onChange={v => this.props.onChange(v as string)}
...
class SpecificAct extends React.Component<SpecificActProps, {}> {
    render() {
        ...
        const value = (this.props.value === null) ? '---'
            : acts[this.props.value].original_name;
        if (this.props.singers)
            acts = acts.filter(a => a.kind === 'Sang');
        acts.sort((a, b) =>
            a.kind !== b.kind ?
            a.kind.localeCompare(b.kind) :
            a.original_name.localeCompare(b.original_name)
        );
        const choices = [{key: null as string | null, name: '---', conflicts: false}].concat(acts);
...
class PlannerRow extends React.Component<PlannerRowProps, {}> {
    ...
    renderColumn(idx: number | "others", people: string[]) {
        if (idx === 'others') {
            const choices = this.props.revue.actors.map(
                actor => {
        ...
        const column = this.props.columns[idx];
        const act = (column.key in this.props.value.columns) ? this.props.value.columns[column.key] : null;
...
class Planner extends React.Component<PlannerProps> {
    ...
    render() {
        ...
        for (let j = 0; j < columns.length; j += 1) {
            header.push(
                <th key={j}>
                    <input type="checkbox" onChange={() => songChange(j)}
        ...
        const cols = header.map(o => <col width="*" key={o.key == null ? "null" : o.key} />);

        const plannerRowColumns = columns.map(
            (name, j) => ({key: name, singers: this.state.songFlags[j]})
        );
        ...
        for (let i = 0; i < this.state.rows; i += 1) {
            ...
            const v = this.state.rowData[i] ? this.state.rowData[i] : {columns: {}, others: []};
            rows.push(
        ...
        const csv = csvRows.map(function (r) { return r.join('\t') + '\n'; }).join('');
        const dataURI = 'data:text/csv;base64,' + window.btoa(encode_utf8(csv));
        const download = <a href={dataURI} download={'oeveplan.csv'}>Download CSV</a>;

        return <div>
            Steder: <input value={this.state.columns} onChange={e => this.setState({columns: e.target.value})} />
            <div>
                Afbud:
